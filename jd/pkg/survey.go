...
{{- else}}
  {{- "  "}}{{- color "cyan"}}[上下移动选择{{- if and .Help (not .ShowHelp)}}, {{ .Config.HelpInput }} for more help{{end}}]{{color "reset"}}
  {{- "\n"}}
...
func (s *Select) filterOptions(config *survey.PromptConfig) []core.OptionAnswer {
	// the filtered list
