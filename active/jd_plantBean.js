...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://api.yqqy.top/jd/getCode?active=jd_bean&num=${randomCount}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
...
function submitCode() {
  return new Promise(async resolve => {
    $.get({url: `https://api.yqqy.top/jd/submit?active=jd_bean&code=${$.myPlantUuid}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
